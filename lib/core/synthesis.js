...
class Synthesis extends FFBase {
  ...
  addXfadeAndAlphaInput() {
    ...
    forEach(scenes, (scene, index) => {
      ...
      if (isAlpha(file)) {
        this.command.addInputOption(['-c:v', 'libvpx-vp9']);
      }
  ...
  getDefaultOutputOptions() {
    ...
    const ext = conf.getVal('ext');
    const vprofile = conf.getVal('vprofile');
    const isAlpha = Utils.isAlpha(ext);
    // 结果视频
    let opts = []
      // misc
      .concat([
      ...
      .concat([
        ...
        // 'copy',
        '-vcodec',
        isAlpha ? 'libvpx-vp9': 'libx264', // c:v - H.264
        // '-profile:v',
    ...
    // 透明时 移除profile:v 设置
    if (!isAlpha) {
      opts = opts.concat(['-profile:v', vprofile]);
    }
    return opts;
  }
